...
/**
 * Controller class for handling subscriber requests in the library management system.
 * <p>
 * This controller manages requests such as "Borrow For Subscriber", "Return For Subscriber",
 * and "Register" (new subscriber), and also allows clearing any current input. It also provides
 * functionality to handle barcoded books for borrow operations.
 * </p>
 *
 * <p>Implements {@link Initializable} to set up UI components after the FXML is loaded.</p>
 *
 * @author  
 * @version 1.0
 * @since 2025-01-01
...
public class SubscriberRequestsWindowsController extends BaseController implements Initializable {
    ...
    /**
     * Button to send or finalize the current operation (not used in the provided code, but declared).
     */
    @FXML
    private Button btnSend;

    /**
     * Button to navigate back to the previous window (the librarian window in this case).
     */
    @FXML
    ...
    /**
     * Label for describing the third text field (e.g., "Book Name" or "Phone Number").
     */
    @FXML
    private Label LBL3;
    ...
    /**
     * Label for describing the fifth text field (e.g., "Borrowed At" or "Return Time").
     */
    @FXML
    private Label LBL5;
    ...
    /**
     * A text field typically representing the first piece of data 
     * (e.g., subscriber name or request detail).
     */
    @FXML
    ...
    /**
     * A text field representing the third piece of data (e.g., book name, phone number).
     */
    @FXML
    ...
    /**
     * ComboBox listing subscriber names or other relevant identifiers for the request.
     */
    @FXML
    private ComboBox<String> RequestedByCB;

    /**
     * ComboBox listing the specific request details (e.g., which book is being borrowed).
     */
    @FXML
    ...
    /**
     * A string indicating the current request type (e.g., "Borrow For Subscriber", "Return For Subscriber", "Registers", "Clear").
     */
    ...
    /**
     * Sets up the form for a "Register" request. This allows registering a new subscriber.
     *
     * @throws InterruptedException if waiting for server responses is interrupted.
     */
    ...
    /**
     * Sets up the form for a "Borrow For Subscriber" request, 
     * optionally showing a barcode scanner button if needed.
     *
     * @throws InterruptedException if waiting for server responses is interrupted.
     */
    ...
    /**
     * Updates the label text, toggles visible components, and fetches relevant requests from the server
     * based on the selected request type.
     *
     * @param type the type of request selected (e.g., "Clear", "Registers", "Borrow For Subscriber", "Return For Subscriber").
     * @throws InterruptedException if waiting for server responses is interrupted.
     */
    ...
    /**
     * Processes "Return For Subscriber" requests retrieved from the server and populates the UI fields accordingly.
     */
    ...
    /**
     * Called when the user selects a subscriber name in the {@link #RequestedByCB} ComboBox.
     * Automatically populates the other ComboBox with all relevant requests for that subscriber.
     */
    ...
    /**
     * Called when the user selects a specific request in the {@link #RequestCB} ComboBox.
     * Automatically fills in the text fields with details from that request.
     */
    ...
    /**
     * Formats a request into a string suitable for display in the {@link #RequestCB} ComboBox.
     *
     * @param requestType the type of request ("Registers", "Borrow For Subscriber", or "Return For Subscriber").
    ...
    /**
     * Processes "Borrow For Subscriber" requests retrieved from the server and populates the UI fields accordingly.
     */
    ...
    /**
     * Processes "Register" requests retrieved from the server and populates the UI fields accordingly.
     */
     ...
     * <p>Validates the fields based on the current request type and sends relevant
     * messages to the server. Also updates subscriber history when appropriate.</p>
     *
    ...
    public void acceptRequest(ActionEvent event) throws Exception {
        ...
        if ("Borrow For Subscriber".equals(selectedRequestType)) {
            ...
            if (ChatClient.s1.getStatus().split(":")[1].equals("Frozen at")) {
                showColoredLabelMessageOnGUIAndMakeItDisappearAfterDelay(
                        feedbackLabel,
                        "The subscriber " + ChatClient.s1.getSubscriber_name() + " is currently frozen and can't borrow books.",
                        "-fx-text-fill: blue;",
                        3
                );
                return;
        ...
        // Return For Subscriber
            ...
            if (lostBook) {
                ...
                showColoredLabelMessageOnGUIAndMakeItDisappearAfterDelay(
                        feedbackLabel,
                        "Return request accepted successfully! (Book marked as lost)",
                        "-fx-text-fill: green;",
                        3
                );
                btnAccept.setDisable(true);
                ...
                int numOfDaysOfReturn = clock.timeDateDifferenceBetweenTwoDates(
                        clock.convertDateFormat(expectedReturn),
                ...
                showColoredLabelMessageOnGUIAndMakeItDisappearAfterDelay(
                        feedbackLabel,
                        "Return request accepted successfully! (" + statusOfReturn + ")",
                        "-fx-text-fill: green;",
                        7
                );
                btnAccept.setDisable(true);
    ...
    /**
     * Navigates back to the Librarian Window, disabling barcode usage for the next session.
     *
    ...
    /**
     * Opens the barcode scanner window to retrieve book information for a "Borrow For Subscriber" request.
     *
    ...
    /**
     * Configures the UI for a "Register" request type, fetching any server data as needed.
     *
     * @throws InterruptedException if waiting for server responses is interrupted.
     */
     ...
     * @param selectedRequestType the current request type (e.g., "Borrow For Subscriber").
     ...
     */
    private boolean areAllFieldsFilled(Label feedbackLabel, String selectedRequestType, 
            TextField TXTF1, TextField TXTF2, TextField TXTF3, TextField TXTF4, TextField TXTF5) {
